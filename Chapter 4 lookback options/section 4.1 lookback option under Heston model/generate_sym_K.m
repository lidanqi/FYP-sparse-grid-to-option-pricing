...
coeff_inner = subs(coeff_inner,{'hx', 'hv'},{ hx, hv});
for i=(2:N1)
    for j=(2:N2)     
            x = (i-1) * hx;
            v = (j-1) * hv;
...
% coefficients for boundary points
 ...
 coeff_Smin = subs(coeff_Smin,{'hx', 'hv'},{ hx, hv});
 coeff_Smax = get_coeff(3,2);
 coeff_Smax = subs(coeff_Smax,{'hx', 'hv'},{ hx, hv });
 for j=(2:N2)        
            i = 1; x=0;
            v = (j-1) * hv; 
...
%             temp_coe = subs(coeff_Smax,{'S','v', 'r'},{S,v,r});
 ...
 coeff_vmin = subs(coeff_vmin,{'hx', 'hv'},{ hx, hv });
 coeff_vmax = get_coeff(2,3);
 coeff_vmax = subs(coeff_vmax,{'hx', 'hv'},{ hx, hv });
 for i=(2:N1)    
            j = 1; v=0;
            x = (i-1) * hx;
...
%% Derivatives discretization
    function  second_S=derivative_x2(i,j,k)
     if (i==N1+1)||(i==1)
        second_S =0;
     else 
    ...
    function first_S = derivative_x1(i,j,k)
        if (i==1)
    ...
    function second_Sv = derivative_xv(i,j,k)
        if (i==1 || j==1) || (i==N1+1 || j==N2+1)
            second_Sv=0;
        else
            if cor_Sv>0
            second_Sv = 1/2 *( ( D(locate_D(i+1,j+1,k)) - D(locate_D(i,j+1,k)) - ( D(locate_D(i+1,j,k))-D(locate_D(i,j,k)) ) )/hx/hv...
                      + (D(locate_D(i,j,k))-D(locate_D(i-1,j,k))-(D(locate_D(i,j-1,k))-D(locate_D(i-1,j-1,k))))/hx/hv );
            else
            second_Sv = 1/2 *( ( D(locate_D(i,j+1,k)) - D(locate_D(i-1,j+1,k)) - ( D(locate_D(i,j,k))-D(locate_D(i-1,j,k)) ) )/hx/hv...
...
%% sub functions        
    function result = calculate_coefficients(i_input,j_input)
        i=i_input; j=j_input; 
        % discretization
        second_x=derivative_x2;        second_v=derivative_v2;       
        first_x = derivative_x1;        first_v = derivative_v1;  
        second_xv = derivative_xv;%       second_Sr = derivative_Sr;%       second_vr = derivative_vr;
        % matrix tryout
        matrix_K = (v/2) * (second_x) - (r-q+v/2)*(first_x) ...
                 + cor_Sv*sig_v*v*(first_v) + k_v*(theta_v-v)*(first_v) + 0.5*(sig_v^2)*v*(second_v) ...
...
%% Derivatives discretization
    function  second_S=derivative_x2()
     if (i==3)||(i==1)
        second_S =0;
     else 
    ...
    function first_x = derivative_x1()
        if (i==1)
            first_x = ( D(locate_D(2,j)) - D(locate_D(1,j)) )/ hx;
            return;
        end
        if (i==3)
             % x=xmax=0: first derivative=0
%              first_x = ( D(locate_D(n1+1,j)) - D(locate_D(n1,j)) ) / hx ;
             first_x=0;
        else
             first_x = ( D(locate_D(i+1,j)) - D(locate_D(i-1,j)) )...
                     / hx / 2;  
        end
    end
    ...
    function second_Sv = derivative_xv()
        if (i==1 || j==1) || (i==3 || j==3)
            second_Sv=0;
        else
            if cor_Sv>0
            second_Sv = 1/2 *( ( D(locate_D(i+1,j+1)) - D(locate_D(i,j+1)) - ( D(locate_D(i+1,j))-D(locate_D(i,j)) ) )/hx/hv...
                      + (D(locate_D(i,j))-D(locate_D(i-1,j))-(D(locate_D(i,j-1))-D(locate_D(i-1,j-1))))/hx/hv );
            else
            second_Sv = 1/2 *( ( D(locate_D(i,j+1)) - D(locate_D(i-1,j+1)) - ( D(locate_D(i,j))-D(locate_D(i-1,j)) ) )/hx/hv...
                      + (D(locate_D(i+1,j))-D(locate_D(i,j))-(D(locate_D(i+1,j-1))-D(locate_D(i,j-1))))/hx/hv );      
            end
        end
    end
